...
- [x] **Deploy real** (2026-09-23) — no ar em **Render** (Web Service via Docker, free tier) + **Neon** (PostgreSQL gerenciado): https://jobtrail-49uy.onrender.com. No caminho, corrigido um bug de produção onde assets do build (`.js`/`.css`) eram servidos como `index.html` — causa: `wwwroot` sendo copiado depois do `dotnet restore` no Dockerfile; corrigido reordenando a cópia e apertando o `MapFallbackToFile` para nunca mascarar um asset não encontrado como 200
- [x] Link do projeto ao vivo no README (2026-09-23) — destacado logo no topo, antes da descrição, pensando em quem for avaliar o projeto (recrutador)
- [ ] **Recuperação de senha ("esqueci minha senha")** — hoje não existe nenhum fluxo para isso; usuário que esquecer a senha fica sem acesso à própria conta. Vai exigir pensar em envio de e-mail (serviço externo, provavelmente outro item de free tier a escolher) e um token de reset com expiração curta — não implementado ainda, próximo passo depois do polimento desta fase
